...
/**********************************************************************
 * VobSub2SRT does not support RAR-backed subtitle streams.
 * The old unRAR handling code has been removed for simplicity
 * and to reduce maintenance/memory-safety risk.
 * -C
**********************************************************************/
typedef FILE sub_stream_t;
...
static int vobsub_parse_one_line(vobsub_t *vob, sub_stream_t *fd,
        ...
        if (buf) {
			...
			vob = NULL;
                ...
                while (vob->spu_streams_current-- > 0) {
                    ...
                    if (vobsubid == (int)vob->spu_streams_current ||
...
int vobsub_get_id_by_index(void *vobhandle, unsigned int index)
{
    ...
    for (i = 0, j = 0; i < vob->spu_streams_size; ++i)
        if ((int)i == vobsubid || vob->spu_streams[i].packets_size > 0) {
            if (j == index)
                return i;
            ++j;
        }
...
int vobsub_get_index_by_id(void *vobhandle, int id)
{
    vobsub_t *vob = vobhandle;
    int i, j;
    if (vob == NULL || id < 0 || (unsigned int)id >= vob->spu_streams_size)
        return -1;
