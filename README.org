...
does not use any preprocessing. Use the latest version for better results.
You can try adjusting =--min-width= and =--min-height=, or =--y-threshold=.
The =--blacklist= option can be useful in some cases too. Otherwise correct
mistakes in the =.srt= files with a text editor or a subtitle editor.

* Tested with:
- Debian GNU/Linux 5 "Lenny" (fully updated final point release)
- Debian GNU/Linux 6.0.10 "Squeeze"
- Fedora 33

See master branch for:
- Arch Linux
- Debian 9-14
- Fedora 34-44+Rawhide


* Building
You will need Tesseract, CMake 2.6.0+ and GCC 4.3+ to build it, as well as a few imaging libraries.
...
Note that the support for tesseract below version 2.4 is deprecated and may not work.
...
#+BEGIN_EXAMPLE
  sudo apt-get install libpng-dev libwebp-dev libgif-dev zlib1g-dev libjpeg-dev binutils-gold
#+END_EXAMPLE
...
*NOTE: This PPA has not been updated since 2013. Please build from source.*
...
** NOTE: Homebrew and ebuilds are currently unsupported.
...
#+BEGIN_EXAMPLE
  ...
  brew install --HEAD https://github.com/csingleplus/VobSub2SRT/raw/legacy/packaging/vobsub2srt.rb
#+END_EXAMPLE
...
#+BEGIN_EXAMPLE
  ...
  wget https://github.com/csingleplus/VobSub2SRT/raw/legacy/packaging/vobsub2srt-9999.ebuild
...
This fork is currently maintained at [[https://github.com/csingleplus/]],
however this branch exists purely for compatibility with older systems.

If you have problems with a specific subtitle file then please check if
it works in mplayer or mpv first.

For bug reports please run =vobsub2srt= with the =--verbose= option and copy
and paste the first ~50 lines to the bug report.
