# -*- mode:org; mode:auto-fill; fill-column:80; coding:utf-8; tab-width:0; -*-
...
* Building
You need Tesseract, CMake and GCC to build =vobsub2srt=.
With a Debian-based distribution, you can install the dependencies with:

#+BEGIN_EXAMPLE
  sudo apt-get install libtiff5-dev libtesseract-dev build-essential cmake pkg-config
#+END_EXAMPLE
...
Note that support for tesseract 2 is deprecated and may not work in the
future!
...
* NOTE: This PPA is no longer maintained and it is recommended to build from current source.
...
** This is also no longer maintained. I am aware of a fork with a fixed formula,
so feel free to seek that out, otherwise just build from source or stop using a
Mac.
...
#+BEGIN_EXAMPLE
  ...
  brew install --HEAD https://github.com/csingleplus/VobSub2SRT/raw/master/packaging/vobsub2srt.rb
#+END_EXAMPLE


** Gentoo ebuild
You run Gentoo. I doubt you need further instructions for this.
...
#+BEGIN_SRC
	palette: 000000,ffffff,000000,000000,000000,000000,ffffff,000000,000000,000000,000000,ffffff,000000,ffffff,000000,ffffff
#+END_SRC

For bug reports please run =vobsub2srt= with the =--verbose= option set to 2,
and copy and paste the first 50~ lines to the bug report.
